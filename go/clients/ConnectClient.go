...
import (
	...
	"github.com/hiveot/hivekit/go/clients/hiveotsseclient"
	"github.com/hiveot/hivekit/go/clients/httpclient"
	"github.com/hiveot/hivekit/go/clients/wssclient"
	"github.com/hiveot/hivekit/go/messaging"
	"github.com/hiveot/hivekit/go/servers/discoserver"
	"github.com/hiveot/hivekit/go/servers/hiveotsseserver"
...
func ConnectWithPassword(
	...
	if connectURL == "" {
		var records []*discoclient.DiscoveryResult

		records, err = discoclient.DiscoverWithDnsSD(
	...
	authCl := authclient.NewAuthClient(parts.Host, caCert, timeout)
...
func ConnectWithToken(
	...
	cc, err = NewClient(clientID, caCert, connectURL, timeout)
	if err == nil {
...
// NewClient returns a new unconnected client ready for connecting to a
// hiveot server.
...
func NewClient(
	...
	if connectURL == "" {
		...
		discoList, err := discoclient.DiscoverWithDnsSD(
