...
func (req *RequestMessage) CreateErrorResponse(err error) (errResp *ResponseMessage) {
	errResp = NewResponseMessage(
		req.Operation, req.ThingID, req.Name, nil, err, StatusFailed, req.CorrelationID)
	return errResp
}

// CreateResponse is a helper to easily create a response from a request
//
// If the response is to an invokeaction request and not query(all)actions
// then it is forwarded to CreateActionResponse.
//
//	value contains the response data
//	err is set when the request has failed. In that case state is failed and the response
