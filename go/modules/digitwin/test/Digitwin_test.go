...
func TestReadDigitwinProperty(t *testing.T) {
	...
	co.SetNotificationHook(func(notif *msg.NotificationMessage) {
		...
		assert.Equal(t, thing1ID, notif.ThingID)
	...
	ag.PubProperty(thing1ID, prop1Name, prop1Value)
...
func TestWriteDigitwinProperty(t *testing.T) {
	...
	err = directoryclient.UpdateTD(directoryapi.DefaultDirectoryServiceID, td1Json, ag.HandleRequest)
