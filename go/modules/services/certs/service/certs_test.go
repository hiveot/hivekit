...
func TestX509ToFromPem(t *testing.T) {
	testCerts := service.CreateTestCertBundle()
...
func TestSaveLoadX509Cert(t *testing.T) {
	...
	cert, err := service.LoadX509CertFromPEM(caPemFile)
	assert.NoError(t, err)
...
//	testCerts := service.CreateTestCertBundle()
//	// save the test x509 cert
//	err := service.SaveX509CertToPEM(testCerts.CaCert, certFile)
//	assert.Error(t, err)
//
//	_, err = service.LoadX509CertFromPEM(certFile)
//	assert.Error(t, err)
//
//	// save the test x509 part of the TLS cert
//	err = service.SaveTLSCertToPEM(testCerts.ServerCert, certFile, keyFile)
...
//
//	// load back the x509 part of the TLS cert
//	_, err = service.LoadTLSCertFromPEM(certFile, keyFile)
//	assert.Error(t, err)
//
//}
