...
// ValueStore is an in-memory value-cache
...
func (store *ValueStore) GetNrThings() int {
	store.mux.RLock()
...
func (store *ValueStore) ReadAction(thingID string, name string) (action *msg.NotificationMessage) {
	store.mux.RLock()
	defer store.mux.RUnlock()
...
func (store *ValueStore) ReadEvent(thingID string, name string) (event *msg.NotificationMessage) {
	store.mux.RLock()
	defer store.mux.RUnlock()
	tn, found := store.Things[thingID]
	if found {
...
func (store *ValueStore) ReadProperty(thingID string, name string) (prop *msg.NotificationMessage) {
	store.mux.RLock()
	defer store.mux.RUnlock()
...
func (store *ValueStore) ReadMultipleProperties(
...
func (store *ValueStore) RemoveProperty(thingID string, name string) {
	store.mux.Lock()
...
func (store *ValueStore) WriteValue(notif *msg.NotificationMessage) {
	store.mux.Lock()
	defer store.mux.Unlock()
...
func NewVCacheStore() *ValueStore {
	store := &ValueStore{
