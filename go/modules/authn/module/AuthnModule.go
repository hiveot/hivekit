...
type AuthnModule struct {
	...
	userHttpHandler *authnserver.UserHttpHandler
}
...
func (m *AuthnModule) GetConnectURL() string {
	...
	loginURL := path.Join(baseURL, authnserver.HttpPostLoginPath)
	return loginURL
}
...
func (m *AuthnModule) HandleRequest(req *msg.RequestMessage, replyTo msg.ResponseHandler) error {
	...
	if req.ThingID == authnserver.AuthnAdminServiceID {
		return authnserver.HandleAuthnAdminRequest(m, req, replyTo)
	} else if req.ThingID == authnserver.AuthnUserServiceID {
		return authnserver.HandleAuthnUserRequest(m, req, replyTo)
	} else {
		// forward
...
func (m *AuthnModule) Start() (err error) {

	if m.httpServer != nil {
		m.userHttpHandler = authnserver.NewUserHttpHandler(m.authenticator, m.httpServer)
	}
