...
func TestNoCA(t *testing.T) {
	...
	cl := httpapi.NewTLSClient(testAddress, authBundle.ClientCert, nil, 0)
	assert.NoError(t, err)
	...
	// No authentication
	cl = httpapi.NewTLSClient(testAddress, nil, nil, 0)
...
func TestNoClientCert(t *testing.T) {
	cl := httpapi.NewTLSClient(testAddress, nil, authBundle.CaCert, 3)
	cl.Close()
}
...
func TestBadClientCert(t *testing.T) {
	...
	cl := httpapi.NewTLSClient(testAddress, otherTLS, authBundle.CaCert, 0)
