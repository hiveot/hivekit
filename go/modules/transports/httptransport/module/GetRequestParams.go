...
func GetClientIdFromContext(r *http.Request) (clientID string, err error) {
	ctxClientID := r.Context().Value(httptransport.ClientContextID)
	if ctxClientID == nil {
...
func GetRequestParams(r *http.Request) (reqParam httptransport.RequestParams, err error) {
	...
	correlationID := r.Header.Get(httptransport.CorrelationIDHeader)
	...
	headerCID := r.Header.Get(httptransport.ConnectionIDHeader)
	if headerCID == "" {
	...
	reqParam.ThingID = chi.URLParam(r, httptransport.ThingIDURIVar)
	reqParam.Name = chi.URLParam(r, httptransport.NameURIVar)
	reqParam.Op = chi.URLParam(r, httptransport.OperationURIVar)
	if r.Body != nil {
