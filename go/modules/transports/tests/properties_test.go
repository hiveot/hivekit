...
func TestObservePropertyByConsumer(t *testing.T) {
	...
	defer cc1.Close()
	cc2, cl2, _ := NewTestConsumer(tpauthn, testClientID1)
	defer cc2.Close()
	...
	cl2.SetNotificationHandler(func(ev *msg.NotificationMessage) {
...
func TestPublishPropertyByAgent(t *testing.T) {
	...
	agConn1, ag1, _ := NewTestAgent(tpauthn, testAgentID1)
	defer agConn1.Close()
...
func TestReadProperty(t *testing.T) {
	...
	cc1, consumer1, _ := NewTestConsumer(tpauthn, testClientID1)
	defer cc1.Close()
