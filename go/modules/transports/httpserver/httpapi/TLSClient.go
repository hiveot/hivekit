...
type TLSClient struct {
	...
	// customHeaders to include in each request
...
func (cl *TLSClient) Delete(path string) (resp []byte, httpStatus int, err error) {
	...
	resp, httpStatus, _, err = cl.Send("DELETE", path, nil, "", nil)
...
func (cl *TLSClient) Get(path string) (resp []byte, httpStatus int, err error) {
	resp, httpStatus, _, err = cl.Send("GET", path, nil, "", nil)
	return resp, httpStatus, err
}
...
func (cl *TLSClient) Send(
	method string, path string, body []byte, contentType string, qParams map[string]string) (
	...
	if cl == nil || cl.httpClient == nil {
		err = fmt.Errorf("send: '%s'. Client is not started", fullURL)
		return nil, http.StatusInternalServerError, nil, err
	}
	// slog.Debug("TLSClient.Send", "method", method, "requrl", fullURL)
	...
	if httpStatus == 401 {
		...
		slog.Error("Send returned internal server error", "fullURL", fullURL, "err", err.Error())
	} else if err != nil {
		err = fmt.Errorf("Send: Error %s %s: %w", method, fullURL, err)
	}
...
func (cl *TLSClient) Patch(
	...
	// careful, a double // in the path causes a 301 and changes POST to GET
	resp, statusCode, _, err = cl.Send(http.MethodPatch, path, body, "", nil)
...
func (cl *TLSClient) PostForm(path string, formData map[string]string) (
	...
	body := form.Encode()
	resp, statusCode, _, err = cl.Send(
		http.MethodPost, path, []byte(body), "application/x-www-form-urlencoded", nil)
	return resp, statusCode, err
}
...
func (cl *TLSClient) Put(path string, body []byte) (
	...
	// careful, a double // in the path causes a 301 and changes POST to GET
	resp, statusCode, _, err = cl.Send(http.MethodPut, path, body, "", nil)
	return resp, statusCode, err
}
