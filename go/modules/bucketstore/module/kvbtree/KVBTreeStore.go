...
//
// This uses josh baker's b-tree implementation: github.com/tidwall/btree
//
...
// scales up to 1M records. Pretty much all you need for basic KV-storage.
//
...
//
// if storePath is a directory a file named "kvbtree_store.json" is used there.
//
...
func readStoreFile(storePath string) (docs map[string]map[string][]byte, err error) {
	...
	if storePathStat, statErr := os.Stat(storePath); statErr == nil {
		if storePathStat.IsDir() {
			// use default file in the directory
	...
	if err == nil {
		if len(rawData) > 100000000 {
			slog.Warn("KVBTreeStore size getting large.",
				"file", storeFile, "size in MB", len(rawData)/1024/1024)
		}
...
// Note this is not concurrent safe. Callers must lock or create a shallow copy of the bucket.
//
...
func writeStoreFile(storePath string, docs map[string]map[string][]byte) error {
	...
	if os.IsNotExist(err) {
		...
		slog.Info("Store folder does not exist. Creating it now.", "storeDir", storeDir)
		err = os.Mkdir(storeDir, 0700)
	} else if !storeDirStat.IsDir() {
		// store exists but not a directory
		err = fmt.Errorf("store directory '%s' is not a directory", storeDir)
	}
...
func (store *KVBTreeStore) GetBucket(bucketID string) (bucket bucketstore.IBucket) {
	...
	store.mutex.Lock()
	kvBucket := store.buckets[bucketID]
	if kvBucket == nil {
