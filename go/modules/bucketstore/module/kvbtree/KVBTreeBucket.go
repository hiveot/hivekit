...
func (kvb *KVBTreeBucket) Close() (err error) {

	slog.Debug("closing bucket", "bucketID", kvb.BucketID)
...
func (kvb *KVBTreeBucket) Cursor() (bucketstore.IBucketCursor, error) {
	...
	iter := kvb.kvtree.Iter()
	cursor := NewKVCursor(kvb, iter)
	return cursor, nil
}

// Delete a document from the bucket
// Also succeeds if the document doesn't exist
func (kvb *KVBTreeBucket) Delete(key string) error {
	kvb.mutex.Lock()
	defer kvb.mutex.Unlock()

	slog.Info("Deleting key from bucket", "key", key, "bucketID", kvb.BucketID)
	kvb.kvtree.Delete(key)
	kvb.updated(kvb)
	return nil
}

// Export returns a shallow copy of the bucket content
func (kvb *KVBTreeBucket) Export() map[string][]byte {
	kvb.mutex.RLock()
	defer kvb.mutex.RUnlock()
...
func (kvb *KVBTreeBucket) Get(key string) (val []byte, err error) {
	var found bool
	kvb.mutex.RLock()
	defer kvb.mutex.RUnlock()

	val, found = kvb.kvtree.Get(key)
	if !found {
...
func (kvb *KVBTreeBucket) GetMultiple(keys []string) (docs map[string][]byte, err error) {
...
//	BucketID string, jsonPath string, orderedKeys []string) (cursor bucketstorage.IBucketCursor, err error) {
//
...
func (kvb *KVBTreeBucket) ID() string {
	return kvb.BucketID
}

// increment the ref counter when a new bucket is requested
func (kvb *KVBTreeBucket) incrRefCounter() {
	kvb.mutex.Lock()
	defer kvb.mutex.Unlock()
	kvb.refCount++
}
...
func (kvb *KVBTreeBucket) Set(key string, doc []byte) error {
	if key == "" {
	...
	kvb.mutex.Lock()
	defer kvb.mutex.Unlock()
	kvb.kvtree.Set(key, Clone(doc))
	kvb.updated(kvb)
	return nil
}

func (kvb *KVBTreeBucket) setUpdateHandler(handler func(bucket *KVBTreeBucket)) {
	kvb.updated = handler
}

// SetMultiple writes a batch of key-values
// Values are copied
func (kvb *KVBTreeBucket) SetMultiple(docs map[string][]byte) (err error) {
	// store the document and object
	kvb.mutex.Lock()
	defer kvb.mutex.Unlock()
	for k, v := range docs {
		kvb.kvtree.Set(k, bytes.NewBuffer(v).Bytes())
	}
	kvb.updated(kvb)
	return nil
}
