...
type ReadHistoryMsgHandler struct {
	...
	histStore historyapi.IHistoryService
...
func (svc *ReadHistoryMsgHandler) HandleRequest(req *msg.RequestMessage, replyTo msg.ResponseHandler) (err error) {
	...
	switch req.Name {
	case historyapi.CreateCursorMethod:
		resp, err = svc.CreateCursor(req)
	case historyapi.CursorFirstMethod:
		resp, err = svc.First(req)
	case historyapi.CursorLastMethod:
		resp, err = svc.Last(req)
	case historyapi.CursorNextMethod:
		resp, err = svc.Next(req)
	case historyapi.CursorNextNMethod:
		resp, err = svc.NextN(req)
	case historyapi.CursorPrevMethod:
		resp, err = svc.Prev(req)
	case historyapi.CursorPrevNMethod:
		resp, err = svc.PrevN(req)
	case historyapi.CursorReleaseMethod:
		resp, err = svc.ReleaseCursor(req)
	case historyapi.CursorSeekMethod:
		resp, err = svc.Seek(req)
	case historyapi.ReadHistoryMethod:
...
func NewReadHistoryMsgHandler(histStore historyapi.IHistoryService) (svc *ReadHistoryMsgHandler) {
