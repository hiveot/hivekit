...
type TransportManager struct {
	...
	httpBasicServer *httpbasic.HttpWoTBasicServer

	// transport protocol bindings in order of startup
	servers []servers.IMessageServer
	// transport protocol bindings by protocol ID
	serversByProtocol map[string]servers.IMessageServer
...
func (svc *TransportManager) GetServer(protocolType string) servers.IMessageServer {
...
func NewTransportManager(cfg *servers.ProtocolsConfig,
	...
	svc = &TransportManager{
		...
		servers:             make([]servers.IMessageServer, 0),
		serversByProtocol:   make(map[string]servers.IMessageServer),
	...
	if cfg.EnableHiveotAuth || cfg.EnableWSS || cfg.EnableHiveotSSE {
		...
		svc.httpBasicServer = httpbasic.NewWoTHttpBasicServer(
