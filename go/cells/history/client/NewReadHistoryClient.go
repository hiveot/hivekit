...
// ReadHistoryClient client for messaging the history service.
...
//
// This client only handles the messaging. It must be linked to a transport client
...
func NewReadHistoryClient(sink api.IHiveCell) *ReadHistoryClient {
	...
	histCl := &ReadHistoryClient{
		histThingID: history.HistoryServiceCellType,
	}
	if sink != nil {
...
//
//	invokeAction is the TD invokeAction for the invoke-action operation of the history service
func NewReadHistoryClientFactory(f api.ICellFactory, md *api.CellDefinition) (api.IHiveCell, error) {
