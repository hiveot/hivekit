...
// TLSServerImpl is a hiveot cell providing a TLS HTTPS server.
// Intended for use by HTTP based application protocols.
// This implements IHttpServer and IHiveCell interfaces.
//
// Note that this does not implement the ITransportServer interface as this service
// just provides the http server for use by transport servers.
type TLSServerImpl struct {
	*cells.HiveCellBase
	...
	// Intended for Http cells to add their routes
	router *chi.Mux
