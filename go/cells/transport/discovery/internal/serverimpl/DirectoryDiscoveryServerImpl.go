...
// DirectoryDiscoveryServerImpl serves a TDD over http and publishing a corresponding
...
// chain sends a corresponding action request.
...
// NewDirectoryDiscoveryServerImpl creates a new discovery server instance.
//
// The instanceID is used in publishing the discovery record. It is also the
// thingID of the instance, and defaults to DiscoveryServerCellType-{shortid}.
//
//	instanceID of the discovery server.
...
func NewDirectoryDiscoveryServerImpl(instanceID string,
	...
	if instanceID == "" {
		instanceID = discovery.DirectoryDiscoveryServerCellType + "-" + shortid.MustGenerate()
	}
