...
// ThingDiscoveryServerImpl serves a Thing TD over http and publishing a corresponding
...
// Intended for use in a cell chain where a device publishes its TD for discovery.
func (m *ThingDiscoveryServerImpl) HandleRequest(req *msg.RequestMessage, replyTo msg.ResponseHandler) (err error) {
	...
	if req.Operation == td.OpInvokeAction {
		...
		case directory.UpdateThingAction, directory.CreateThingAction:
			...
			// With a discovery server in the chain this is used to publish a Thing discovery
			// record.
			//
			tdJson := req.ToString(0)
			err = m.ServeThingTD(tdJson)
...
// NewThingDiscoveryServerImpl creates a new Thing discovery server instance.
//
// The instanceID is used in publishing the discovery record. It is also the
// thingID of the instance, and defaults to ThingDiscoveryServerCellType-{shortid}.
//
//	instanceID of the discovery server, or "" for the default.
...
func NewThingDiscoveryServerImpl(instanceID string,
	...
	if instanceID == "" {
		instanceID = discovery.ThingDiscoveryServerCellType + "-" + shortid.MustGenerate()
	}
	m := &ThingDiscoveryServerImpl{
		HiveCellBase: cells.NewHiveCellBase(instanceID, 0),
