...
This transport passes RRN messages as-is between client and server using a bi-directional gRPC stream.

## Status

This transport is functional but breaking changes can be expected.
...
The transport configuration consists of:
...
```go
	...
	err := grpcClient.SetAuthToken(clientID, token)
...
This is intended for educational and experimental purpose. 

## Usage

1. Create and start the transport server 
