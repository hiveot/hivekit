...
// Properties that can be exposed in a TD
...
//
// To initialize: call Init(ID, sink, connectURL)
type TransportServerBase struct {
	*cells.HiveCellBase

	// appRequestHook is the application handler of requests addressed to this server's thingID.
	//
	// HandleRequest will invoke this callback or forward requests not destined for
	// this transport (transport ID != request.ThingID) to requestSink.
...
//
// This is used when a consumer or service passes the request to this server,
...
func (m *TransportServerBase) HandleRequest(
	...
	// first attempt to process the when targeted at this server.
	// if the request is not for this server then pass it to the remote device
...
// callback, which by default is the sink's HandleResponse.
//
...
// Set the hook to invoke with received requests directed at this server
...
// NewTransportServerBase initializes a server intended for embedding.
//
...
//	connectURL is the URL this transport can be reached at. Used to set TD.Base
