...
// Actions for use in RRN messaging as used in the TM.
...
type IBucketStore interface {
	...
	// GetLocation returns the full storage location path.
	// If the store is a single self-contained file, like kvbtree then this is the full path to the file
	// If the store is a multi-file store, like pebble, this is the directory to the store files.
	// If the store is remote, like redis, this is the URL to the storage server.
...
type IBucketCursor interface {
	...
	// NextN moves the cursor to the next N places from the current object
	...
	// PrevN moves the cursor back N places from the current object and returns a map with
