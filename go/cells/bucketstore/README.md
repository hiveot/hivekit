# bucketstore - Key-Value Storage Service

Storage cells provides concurrent safe, key-value, iterative storage for IoT applications with the focus on simplicity. It defines a simple to use standardized key-value API with multiple backend implementations. It supports ordered-key seek and iteration.
...
This service can be used as a stand-alone backend or its stores can be created via the factory function by cells that need an embedded store.

## Status

This service is in alpha. It is functional but breaking changes, although unlikely, might stil happen.
...
The kvbtree backend is an embedded in-memory store using a btree as a store per client. Data is serialized and persisted to one file per store containing all buckets. Data is periodically written to disk after modifications are made.

This backend is exceptionally fast for both reading and writing. A read and a write takes less than 1 usec per record, so a speed of a million read/writes per second is possible.
...
This store is best suited for limited amount of data, based on memory, that is frequently read and updated. The recommended data limit is 100MB.
