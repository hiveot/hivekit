...
func (ag *Agent) PubActionProgress(req messaging.RequestMessage, value any) error {
	status := messaging.ActionStatus{
...
func (ag *Agent) SendNotification(notif *messaging.NotificationMessage) error {
	return ag.GetConnection().SendNotification(notif)
}

// SendResponse sends a response for a previous request
func (ag *Agent) SendResponse(resp *messaging.ResponseMessage) error {
...
func NewAgent(cc messaging.IConnection,
	...
	if timeout == 0 {
		timeout = consumer.DefaultRpcTimeout
	}
	...
	agent.Consumer = consumer.NewConsumer(cc, timeout)
