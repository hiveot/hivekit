...
import type IAgentConnection from "../../messaging/IAgentConnection.ts";
import {
  type RequestHandler,
  type ResponseHandler,
  ConnectionStatus,
} from "../../messaging/IConsumerConnection.ts";
import {
  NotificationMessage,
  RequestMessage,
  ResponseMessage,
} from "../../messaging/Messages.ts";
...
export default class HttpSSEClient implements IAgentConnection {
  ...
  // the auth token set by SetAuthToken
  authToken: string;
  ...
  async connect(): Promise<http2.ClientHttp2Session> {
    ...
    this._http2Session.on("connect", (ev) => {
      console.warn(
        "http/2 connected to server, cid=",
        this._cid,
        "url",
        this._baseURL,
      );
  ...
  async connectWithPassword(password: string): Promise<string> {
    ...
    const resp = await this.pubMessage(
      "POST",
      HttpPostLoginPath,
      loginArgs,
      "",
  ...
  async SetAuthToken(authToken: string): Promise<string> {
  ...
  onSseConnection(status: ConnectionStatus) {
    ...
    if (this.sseConnStatus === ConnectionStatus.Connected) {
      ...
      this._sseClient = undefined;
      this.SetAuthToken(this.authToken).then();
    }
  }
  ...
  onRequest(req: RequestMessage): ResponseMessage | null {
    ...
    try {
      if (this.requestHandler) {
        ...
        const err = Error(
          `onRequest: received request but no handler registered: ${req.operation}`,
        );
        hcLog.warn(err);
      ...
      const err = Error(
        `Error handling request sender=${req.senderID}, messageType=${req.operation}, thingID=${req.thingID}, name=${req.name}, error=${e}`,
      );
      hcLog.warn(err);
      resp = req.createResponse(null, err);
    }
  ...
  async pubMessage(
    ...
    if (!this._http2Session || this._http2Session.destroyed) {
      hcLog.warn(
        "http2.pubMessage but connection is closed. Attempting to reconnect",
      );
      await this.connect();
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!this._http2Session || this._http2Session.closed) {
        ...
        reject(
          new Error("pubMessage: Unable to send. http2 connection was closed"),
        );
      } else {
        ...
        h2req.on("response", (r) => {
          if (r[":status"]) {
            ...
            if (statusCode >= 400) {
              hcLog.warn(
                `pubMessage '${path}' returned status code '${statusCode}'`,
              );
            }
          }
        });
        ...
        h2req.on("end", () => {
          ...
          if (statusCode >= 400) {
            hcLog.warn(
              `pubMessage http/2 connection closed with status code  ${statusCode}`,
            );
  ...
  pubTD(td: TD) {
    ...
    this.invokeAction(
      ...
      ThingDirectoryUpdateThingMethod,
      tdJSON,
    )
  ...
  async sendRequest(req: RequestMessage): Promise<ResponseMessage> {
    return new Promise((resolve, _reject): void => {
      ...
      if (href) {
        ...
        this.pubMessage("POST", href, req.input, req.correlationID).then(
          (reply: string) => {
  ...
  sendResponse(resp: ResponseMessage) {
    // responses can only be sent to the fixed endpoint
    this.pubMessage("POST", HiveOTPostResponseHRef, resp, resp.correlationID)
      .then()
      .catch((e) => {
  ...
  subscribe(dThingID: string, name: string): void {
    ...
    this.sendRequest(req).then().catch();
  }
